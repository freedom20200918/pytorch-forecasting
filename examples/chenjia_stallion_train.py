...
study = optimize_hyperparameters(
    train_dataloader,  # 训练数据加载器
    val_dataloader,  # 验证数据加载器
    model_path="optuna_test",  # 模型保存路径（每次 trial 的最佳模型会保存到这里）
    n_trials=200,  # 最大试验次数（即最多搜索 200 组超参数组合）
    max_epochs=50,  # 每组超参数最多训练的 epoch 数
    gradient_clip_val_range=(0.01, 1.0),  # 梯度裁剪值搜索范围
    hidden_size_range=(8, 128),  # 模型隐藏层维度搜索范围
    hidden_continuous_size_range=(8, 128),  # 连续变量隐藏层维度搜索范围
    attention_head_size_range=(1, 4),  # 注意力头数搜索范围
    learning_rate_range=(0.001, 0.1),  # 学习率搜索范围
    dropout_range=(0.1, 0.3),  # dropout 概率搜索范围

    # Trainer 的额外参数配置
    trainer_kwargs=dict(
        limit_train_batches=30,  # 每次只用前 30 个 batch 做训练，加快调参速度
        log_every_n_steps=1,     # 每训练一个 step 打印一次日志（方便观察进度）
        enable_progress_bar=True # 显示训练进度条（tqdm），增强可视化体验
    ),

    reduce_on_plateau_patience=4,  # 如果验证集损失 4 个 epoch 没有提升，则自动降低学习率
    use_learning_rate_finder=False  # 不启用自动学习率搜索功能
)
